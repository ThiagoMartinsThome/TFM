...
def tradebot(event, context):
     ...
     def get_positions(api=api):
          ...
          return df_pf, portfolio_value, float(buying_power), float(cash)
     ...
     def get_top_stocks(cash, df_pf, pf_size=20):
          ...
          df_top_stocks = df_top_stocks.loc[df['date'] == pd.to_datetime(current_data_date)]
          df_top_stocks = df_top_stocks.sort_values(by='predictions', ascending=False)#.head(portfolio_size)
          ...
          df_buy = df_buy.loc[df_buy['date'] == current_data_date].sort_values(by='ticker')
          ...
          return round(df_buy[['ticker', 'close', 'quantity', 'amount_held']], 2), sell_list, df, df_pred, current_data_date
          ...
          if df_stock_diff.shape[0] > 0:
               ...
               if df_stock_diff_sale.shape[0] > 0:
                    if df_sell is not None:
                         ...
                         df_sell_final['share_amt_change'] = df_sell_final['share_amt_change'].fillna(df_sell_final['quantity'])
     ...
     def strategy_log(df_buy_new, df_sell_final):
          ...
          if df_buy_new is not None and df_sell_final is not None:
               df_buy_new['action'] = 'buy'
               df_sell_final['action'] = 'sell'
               position_df = pd.concat([df_buy_new, df_sell_final], ignore_index=True)
          elif df_sell_final is None and df_buy_new is not None:
               df_buy_new['action'] = 'buy'
               position_df = df_buy_new
          elif df_buy_new is None and df_sell_final is not None:
               df_sell_final['action'] = 'sell'
               position_df = df_sell_final
          else:
               position_df = None
     ...
     pd.options.mode.chained_assignment = None
     # Get Positions
     df_pf, portfolio_value, buying_power, cash = get_positions()
     print('df_pf', df_pf)


     # Get_top_stocks
     money = portfolio_value
     df_buy, sell_list, df, df_pred, current_data_date = get_top_stocks(cash=(money), df_pf=df_pf)
     print('df_buy', df_buy)
     ...
     df_sell_final = stock_diffs(
          ...
          df_pf=df_pf,
          df_buy=df_buy)
     print('df_sell_final', df_sell_final)

     # df_buy new
     df_buy_new = buy_new(
          df_pf=df_pf,
          df_buy=df_buy)
     print('df_buy_new', df_buy_new)

     # Sell Order
     sell_order(df_sell_final)

     # Buy Order
     buy_order(df_buy_new)

     # Strategy log
     position_df = strategy_log(df_buy_new, df_sell_final)
