...
if st.sidebar.checkbox("Technical Analysis Viewer"):
    ...
    @st.cache()
    def get_sp500_info():
        # Function gets the stock information
    ...
    # Avoid user choosing more than one symbol
    if len(stock) > 1:
        ...
        for item in stock:
            try:
                ...
                def plot_data(data):
                    ...
                    # Plot Volume and show in millions
                    fig.add_trace(go.Bar(x=data.Date,
...
if st.sidebar.checkbox("Portfolio Management"):
    ...
    # Plot the equity profit/loss evolution
    # Define the figure
    ...
    # Highlight 'Total Profit $' based on the values
    def color(val):
        if val > 0:
            color = 'green'
        elif val < 0:
    ...
    # Apply the color function to the 'Total Porfit $' values
    ...
    if st.sidebar.checkbox("Get Predictions"):
        ...
        if df_buy_new is not None:
            # Display the stocks recommended to buy
            ...
            st.subheader("Buy list: EMPTY !!")
        #st.write(df_sell)
        #st.write(df_sell_final)

        # Display the stocks recommended to sell
        if df_sell_final is not None:
            st.subheader('Sell list: ')
            table_sell = df_sell_final.rename(columns={'ticker': 'Stock',
                                                    'quantity': 'Quantity',
                                                    'close': 'Close'})
            table_sell.reset_index(drop=True, inplace=True)
            st.write(table_sell[['Stock', 'Quantity', 'Close']].style.set_precision(2))
        else:
            st.subheader('Sell list: EMPTY !!')

        # Order bottom
        if market_status == 1:
            if st.sidebar.button("Submit a buy/sell order"):
                tb.sell_order(df_sell_final=df_sell_final)
                tb.buy_order(df_buy_new=df_buy_new)
        ...
        if st.sidebar.checkbox("Portfolio history"):
            # Function get the portfolio history information
            #@st.cache()
            def get_pf_hist():
            ...
            def color_action(val):
                if val == 'buy':
                    ...
                    color = 'red'
                return 'color: {}'.format(color)
            # Display the portfolio history table
