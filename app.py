...
st.sidebar.image('cover2.png', use_column_width=True)
# Project Title
st.sidebar.markdown('# *TradeBot*')
if st.sidebar.checkbox("Project description"):
    """Una breve explicacion del proyecto"""
...
if st.sidebar.checkbox("Portfolio Management"):
    ...
    pf_table = df_pf.rename(columns={'ticker': 'Stock', 'quantity': 'Quantity', 'market_value': 'Market Value'})
    ...
    st.write('**Balance: **', round(float(equity)), "USD")
    ...
    st.dataframe(pf_table.style.applymap(color, subset=['total_profit_USD']).set_precision(2))
    ...
    if st.sidebar.checkbox("Get Predictions"):
        ...
        if st.sidebar.checkbox("Portfolio history"):
            ...
            df_pf_hist = get_pf_hist()
            table_pf_hist = df_pf_hist.rename(columns={'date': 'Date',
                                                       'ticker': 'Stock',
                                                       'quantity': 'Quantity',
                                                       'market_value': 'Market_value',
                                                       'strategy': 'Strategy'})
            st.write(table_pf_hist.style.set_precision(2))
