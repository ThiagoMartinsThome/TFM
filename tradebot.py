...
def get_positions(api=api):
    ...
    return df_pf, portfolio_value, float(buying_power), float(cash), pf_hist_df, float(equity)
...
def get_pf_hist():
    ...
    df_pf_hist = df_pf_hist[['date', 'ticker', 'quantity', 'close', 'action', 'strategy']]
    df_pf_hist.rename(columns={'date': 'Date',
                               'ticker': 'Stock',
                               'quantity': 'Quantity',
                               'close': 'Close',
                               'action': 'Action',
